...
const Calendar1: React.FC = () => {
  ...
  const handleContinue = () => {
    ...
    const allDates = getAllDatesInRange(startDate, rangeEnd);
    
    console.log("Start Date:", startDate.toDateString());
    console.log("End Date:", rangeEnd.toDateString());
    console.log("All Dates:", 
      allDates.map(d => d.toDateString()).join(", ")
    );
              ...
              >
                <div 
                  className={`w-full h-full rounded-full flex items-center justify-center txt-14 ${
                    ...
                    !isFuture && (isStartDate(day) || isEndDate(day)
                      ? 'bg-[#224674] text-white font-semibold' 
                      : isInRange(day)
                        ? 'bg-[#C8E4FC] text-[#224674]'
                        : ''
